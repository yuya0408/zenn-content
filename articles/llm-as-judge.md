...
本題に入る前に、この選定の前提を押さえておく。**採点される側(候補の生成モデル)**と**採点する側(judge)**は、次のように分けた。

**候補の生成モデル(4つ)** ── いずれもローカル運用を前提に選んだ。
...
**judgeに採用したモデル(2つ)** ── 候補には含めていない。

| judge | 位置づけ |
| :-- | :-- |
| `gpt-oss:120b` | ローカルの主judge |
| Claude Opus | 系統の違うクラウド側。クロスチェック用 |

judgeを候補と切り離したのは、採点対象が自分を甘く採点する自己採点バイアスを避けるためだ(この狙いは節2で詳述する)。以下の点数は、すべてこの布陣で採ったものと読んでほしい。

前提はここまで。まず、LLM-as-Judgeが得意なことから。**「明らかに弱い候補を落とす」用途なら、絶対採点(1〜5点)で十分**機能する。
...
もう一点、順位を読むときに割り引くべき事情がある。ローカルjudgeの `gpt-oss:120b` と候補の `gpt-oss:20b` は**同系統のモデル**だという点だ。同系統のjudgeは身内の出力を好意的に採点しやすい(自己選好に近いバイアス)。実際、`gpt-oss:20b` は `gpt-oss:120b` judgeでは2位だが、系統の違うOpusでは3位に落ちている。この順位の一部は品質そのものではなく、judgeと候補が同系統であることの反映かもしれない ── そう疑っておくのが妥当だ。
